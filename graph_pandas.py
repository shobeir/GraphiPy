...
class Graph_Pandas(Graph):
    ...
    def export_all_CSV(self, prefix):
        ...
        for key in n_df.keys():
            n_df[key].to_csv(prefix + "_" + key + "_node.csv", encoding="utf-8", index=False)
        e_df = self.get_edges()
        for key in e_df.keys():
            e_df[key].to_csv(prefix + "_" + key + "_edge.csv", encoding="utf-8", index=False)
    ...
    def create_node(self, node):
        ...
        if node_label not in self.nodes_df:
            ...
            self.nodes_df[node_label] = self.nodes_df[node_label].append(pd.DataFrame.from_dict([vars(node)]))
    ...
    def create_edge(self, edge):
        ...
        if edge_label not in self.edges_df:
            ...
            self.edges_df[edge_label] = self.edges_df[edge_label].append(pd.DataFrame.from_dict([vars(edge)]))
